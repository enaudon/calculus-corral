...
(** [Cannot_unify (tp1, tp2)] indicates that unification failed because [tp1]
    and [tp2] could not be unified. *)
...
(** [get_quants tp] computes the variable quantifiers of [tp]. *)
val get_quants : t -> (Identifier.t * Kind.t) list

(** [is_mu tp] evaluates to [true], if [tp] is a recursive type; otherwise it
    evaluates to [false]. *)
val is_mu : t -> bool
...
(** Inferencer
    ...
    inference functionality, like unification, generalization and instantiation.
...
module Inferencer : sig
  ...
  (** [make_state kn_env tp_env] constructs a state from [kn_env] and [tp_env]. *)
  val make_state : Kind.Environment.t -> Environment.t -> state

  (** [register ~rigid:() state tv] registers the inference variable, [tv], with
      the inference engine. Unless [rigid] is passed, [tv] will be registered as
  ...
  (** [gen_exit state tp] updates the internal state and performs generalization
      ...
      the newly polymorphic variables and their kinds, and c) the
...
(** [to_intl_repr tp] computes an internal representation type which is
    equivalent to [tp]. *)
val to_intl_repr : t -> Isorecursive_types_exp.Type.t
