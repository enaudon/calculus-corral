...
let rec beta_reduce env tp = match tp with
  | Variable id ->
...
module Inferencer : sig
  ...
  let del_type id state =
    {state with type_env = Env.Type.del id state.type_env}
  ...
  let unify state tp1 tp2 =
    ...
    let rec occurs : Id.t -> t -> bool = fun id tp -> match tp with
      ...
      (*
        Only type variables can be recursively bound, so there's no need
        to check [quant <> id] before calling [occurs].
       *)
      | Recursive (_, _, body) -> occurs id body
      | Row_nil -> false
