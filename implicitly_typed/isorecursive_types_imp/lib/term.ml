...
module IR = Isorecursive_types_exp
...
let bind : Loc.t -> Id.t -> t -> t -> bool -> t =
 fun loc id value body recf -> {desc = Binding (id, value, body, recf); loc}

let annot : Loc.t -> t -> Annot.t -> t =
 fun loc tm an -> {desc = Annotation (tm, an); loc}
   ...
   let rec infer env state exp_tp tm =
     ...
     match tm.desc with
       ...
       | Binding (id, value, body, recf) ->
         ...
         let state, value_k =
           if recf then
             infer (Type_env.Term.add id mono_tp env) state mono_tp value
           else
             infer env state mono_tp value
         in
         ...
         ( state,
           fun state ->
             ...
             let value' =
               if recf then
                 let mono_tp' = type_to_ir state mono_tp in
                 IR.Term.fix (IR.Term.abs ~loc id mono_tp' @@ value_k state)
               else
                 value_k state
             in
       ...
       | Case (vrnt, cases) ->
         ...
         let state = unify loc state vrnt_tp @@ Type.vrnt case_tps Opt.none in
         let state, case_ks =
           (* TODO: Use `fold_right2` here. *)
           let s, ks =
         ...
         ( unify loc state exp_tp res_tp,
           fun state ->
             ...
             in
             let vrnt' =
               if Type.is_mu @@ Infer.apply state vrnt_tp then
                 IR.Term.unroll ~loc @@ vrnt_k state
               else
                 vrnt_k state
             in
   ...
   let rec constrain exp_tp tm =
     ...
     match tm.desc with
       ...
       | Binding (id, value, body, recf) ->
         ...
         let poly_tp' = Type.to_intl_repr poly_tp in
         let qs = fst @@ IR.Type.get_forall' poly_tp' in
         let value' =
           if recf then
             let mono_tp' = Type.to_intl_repr mono_tp in
             IR.Term.fix (IR.Term.abs ~loc id mono_tp' value')
           else
             value'
         in
         IR.Term.app
           ~loc
           (IR.Term.abs ~loc id poly_tp' body')
           (coerce tvs qs @@ IR.Term.tp_abs' ~loc qs value')
       | Annotation (tm, an) ->
         Annot.constrain an @@ fun tp -> (constrain tp tm, TC.equals exp_tp tp)
       | Record fields ->
         ...
         TC.exists ~loc Kind.prop (fun res_tp ->
             TC.exists ~loc Kind.prop (fun data_tp ->
                 ...
                 TC.conj_left
                   ...
                   (TC.conj (TC.equals res_tp exp_tp) (TC.equals exp_tp tp))))
         <$> fun (exp_tp, (data_tp, (rest_tp, data'))) ->
       ...
       | Case (vrnt, cases) ->
         ...
         TC.exists_list' ~loc kns (fun tps ->
             ...
             TC.conj_left
               ...
               (TC.conj (TC.equals exp_tp res_tp) (TC.equals vrnt_tp vrnt_tp')))
         <$> fun (_, (vrnt_tp, (_, (vrnt', cases')))) ->
         let vrnt' =
           if Type.is_mu vrnt_tp then
             IR.Term.unroll ~loc vrnt'
           else
             vrnt'
         in
         IR.Term.case ~loc vrnt' cases'
      ...
      let fn_to_string tm =
        match tm.desc with
          | Variable _ | Application _ | Record _ | Projection _ ->
            to_string tm
          | Abstraction _ | Binding _ | Annotation _ | Variant _ | Case _ ->
            to_paren_string tm
      in
    ...
    | Annotation (tm, tp) ->
      let to_string' tm =
        match tm.desc with
          ...
          | Variant _ ->
            to_string tm
          | Abstraction _ | Binding _ | Case _ ->
            to_paren_string tm
      in
      Printf.sprintf "%s : %s" (to_string' tm) (Annot.to_string tp)
    | Record fields ->
