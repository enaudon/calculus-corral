...
let bind : Loc.t -> Id.t -> t -> t -> bool -> t =
    fun loc id value body recf ->
  { desc = Binding (id, value, body, recf); loc }
  ...
  match tm.desc with
    ...
    | Annotation (tm, tp) ->
      let to_string' tm = match tm.desc with
        | Variable _ | Application _ | Annotation _ | Record _
            | Projection _ | Variant _ ->
          to_string tm
