...
let constrain an term_co_fn =
  let rec get_typo an =
    match an with
      | Type tp ->
        tp
      | Universal (_, _, an) | Existential (_, _, an) ->
        get_typo an
  in
  let rec get_universals an =
    match an with
      | Type _ ->
        []
      | Universal (quant, kn, an) ->
        (quant, kn) :: get_universals an
      | Existential (_, _, an) ->
        get_universals an
  in
  let rec get_existentials an =
    match an with
      | Type _ ->
        []
      | Universal (_, _, an) ->
