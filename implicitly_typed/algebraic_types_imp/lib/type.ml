...
module Inferencer : sig
  ...
  module Pools : sig
    ...
    let insert id kn state = {state with pools = IVE.insert id kn state.pools}
  ...
  let make_state kind_env = {sub = Sub.identity; pools = IVE.empty; kind_env}
  ...
  let register state tp kn =
    match tp with
      | Inference_variable (_, id) ->
        Pools.insert id kn state
      | _ ->
...
let to_kind env tp = Inferencer.to_kind (Inferencer.make_state env) tp
