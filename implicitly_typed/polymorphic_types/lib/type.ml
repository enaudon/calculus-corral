...
let raise_poly : string -> 'a =
 fun fn_name ->
   invalid_arg
   @@ Printf.sprintf "%s.%s: unexpected polymorphic type" __MODULE__ fn_name
...
let forall : Id.t -> t -> t = fun quant body -> Universal (quant, body)

let forall' : Id.t list -> t -> t =
 fun quants body -> List.fold_right forall quants body

(* Destructors *)

let get_forall' : t -> Identifier.t list * t =
 fun tp ->
...
module Inferencer : sig
  ...
  module Pools : sig
    ...
    let peek state = IVE.peek state.pools |> Id.Map.keys |> Id.Set.of_list

    let pop state = {state with pools = IVE.pop state.pools}

    let insert id state = {state with pools = IVE.insert id () state.pools}
  ...
  let initial = {sub = Sub.identity; pools = IVE.empty}

  let register state tp =
    match tp with
      | Inference_variable id ->
  ...
  let inst state tp =
    let rec inst env tp =
      match tp with
        ...
        | Function (arg, res) ->
          func (inst env arg) (inst env res)
        | Universal _ ->
