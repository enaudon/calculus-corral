...
let ranks_find : Id.t -> 'a t -> rank = fun id ps ->
  try
...
let pools_find : rank -> Id.t -> 'a t -> 'a = fun rank id ps ->
  try
...
let top : 'a t -> rank = fun ps ->
  Stack.size ps.pools - 1

let insert : rank -> Id.t -> 'a -> 'a t -> 'a t = fun rank id data ps ->
...
let remove : rank -> Id.t -> 'a t -> 'a t = fun rank id ps ->
  let pool = Stack.get rank ps.pools in
