...
let of_lex_pos s e =
  ...
  if s.pos_fname <> e.pos_fname then
    failwith @@ Printf.sprintf "Location.of_lex_pos: different file names";
  if s.pos_lnum > e.pos_lnum then
    failwith @@ Printf.sprintf "Location.of_lex_pos: start line > end line";
  if s.pos_lnum = e.pos_lnum && s.pos_cnum > e.pos_cnum then
    failwith @@ Printf.sprintf "Location.of_lex_pos: start char > end char";
  { file = s.pos_fname;
