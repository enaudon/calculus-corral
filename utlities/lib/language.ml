...
module Repl (Input : Input) = struct
  ...
  let evaluate kn_env tp_env vl_env lexbuf =
    let deep = !deep in
    let evaluate_command (kn_env, tp_env, vl_env) cmd =
      match cmd with
        ...
        | Command.Bind_term (id, tm) ->
          ...
          Printf.printf
            ...
            (Type.to_string tp)
            (Value.to_string vl);
          (kn_env, Type_env.Term.add id tp tp_env, Value_env.add id vl vl_env)
    ...
    in
    try
      List.fold_left evaluate_command (kn_env, tp_env, vl_env) (parse lexbuf)
    with
      | Parsing.Parse_error ->
    ...
    match !mode with
      ...
      | File fs ->
        let eval_file f (kn_env, tp_env, vl_env) =
          ...
          envs
        in
        let envs = (Kind_env.initial, Type_env.initial, Value_env.initial) in
