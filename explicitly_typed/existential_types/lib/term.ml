...
let abs : Loc.t -> Id.t -> Type.t -> t -> t =
 fun loc arg tp body -> {desc = Abstraction (arg, tp, body); loc}

let app : Loc.t -> t -> t -> t =
 fun loc fn arg -> {desc = Application (fn, arg); loc}

let pack : Loc.t -> Type.t -> t -> Type.t -> t =
 fun loc tp1 tm tp2 -> {desc = Pack (tp1, tm, tp2); loc}

let unpack : Loc.t -> Id.t -> Id.t -> t -> t -> t =
 fun loc tp_id tm_id pack body -> {desc = Unpack (tp_id, tm_id, pack, body); loc}
...
let rec to_type (kn_env, tp_env) tm =
  ...
  match tm.desc with
    ...
    | Pack (tp1, tm, tp2) ->
      ...
      let tp2_tv, tp2_tp =
        try Type.get_exists @@ Type.reduce tp_env tp2
        with Invalid_argument _ ->
          error tm.loc "to_type"
          @@ Printf.sprintf
    ...
    | Unpack (tp_id, tm_id, pack, body) ->
      ...
      let pack_tp_tv, pack_tp_tp =
        try Type.get_exists @@ Type.reduce tp_env pack_tp
        with Invalid_argument _ ->
          error tm.loc "to_type"
          @@ Printf.sprintf
               "expected existential type; found '%s'"
...
let rec subst_tp : Id.Set.t -> Type_env.t -> t -> t =
 fun fvs sub tm ->
   let loc = tm.loc in
   match tm.desc with
     | Variable _ ->
     ...
     | Unpack (tp_id, tm_id, pack, body) when Id.Set.mem tp_id fvs ->
       ...
       let sub' = Type_env.Type.add tp_id (Type.var tp_id') sub in
       unpack
         loc
         tp_id'
         tm_id
...
let rec subst_tm : Id.Set.t -> Env.t -> t -> t =
 fun fvs sub tm ->
   let loc = tm.loc in
   match tm.desc with
     | Variable id ->
