  ...
  match tm.desc with
    ...
    | Application (fn, arg) ->
      ...
      let fml_arg_tp, res_tp =
        try Type.get_func (Type.beta_reduce ~deep:() env fn_tp)
 ...
 fun fvs sub tm ->
   ...
   match tm.desc with
     ...
     | Abstraction (arg, tp, body) ->
       abs loc arg tp @@ subst (Id.Set.add arg fvs) (Env.del arg sub) body
     | Application (fn, arg) ->
...
let alpha_equivalent =
  let rec alpha_equiv env tm1 tm2 =
