...
let roll : Loc.t -> Type.t -> t -> t =
 fun loc tp tm -> {desc = Roll (tp, tm); loc}

let unroll : Loc.t -> t -> t = fun loc tm -> {desc = Unroll tm; loc}

let fix : Loc.t -> t -> t = fun loc tm -> {desc = Fix tm; loc}
...
let rec to_type (kn_env, tp_env) tm =
  ...
  match tm.desc with
    ...
    | Term_abs (arg, arg_tp, body) ->
      let arg_kn =
        Type.to_kind kn_env @@ Type.beta_reduce ~deep:() tp_env arg_tp
      in
    ...
    | Type_app (fn, arg) ->
      ...
      in
      let arg_kn =
        Type.to_kind kn_env @@ Type.beta_reduce ~deep:() tp_env arg
      in
    ...
    | Roll (tp, tm) ->
      ...
      let quant, kn, body =
        ...
        with Invalid_argument _ ->
          error tm.loc "to_type"
          @@ Printf.sprintf
               "expected recursive type; found '%s'"
               (Type.to_string tp')
      in
      let ftvs = Id.Set.of_list @@ Kind_env.keys kn_env in
      ...
      if Type.alpha_equivalent ~beta_env:tp_env act exp then
        ...
        @@ Printf.sprintf
             ...
             (Type.to_string act)
    | Unroll tm ->
      let tp = Type.beta_reduce ~deep:() tp_env @@ to_type kn_env tp_env tm in
      (* TODO: See about using the elided [kn] variable. *)
      let quant, _, body =
        try Type.get_mu tp
        with Invalid_argument _ ->
          error tm.loc "to_type"
    ...
    | Fix tm ->
      ...
      let arg_tp, res_tp =
        ...
        with Invalid_argument _ ->
          error tm.loc "to_type"
          @@ Printf.sprintf
               "expected function type; found type '%s'"
               (Type.to_string tp)
      in
      if Type.alpha_equivalent ~beta_env:tp_env arg_tp res_tp then
        res_tp
      else
        error tm.loc "to_type"
        @@ Printf.sprintf
    ...
    | Projection (rcrd, field) ->
      ...
      let fields, _ =
        try Type.get_rcrd @@ Type.beta_reduce ~deep:() tp_env rcrd_tp
        with Invalid_argument _ ->
          error tm.loc "to_type"
          @@ Printf.sprintf
    ...
    | Variant (case, data, tp) ->
      let kn = Type.to_kind kn_env @@ Type.beta_reduce ~deep:() tp_env tp in
      if not (Kind.alpha_equivalent kn Kind.prop) then
        error tm.loc "to_type"
        @@ Printf.sprintf "expected proper kind; found '%s'" (Kind.to_string kn);
      let cases, _ =
        try Type.get_vrnt @@ Type.beta_reduce ~deep:() tp_env tp
        with Invalid_argument _ ->
          error tm.loc "to_type"
          @@ Printf.sprintf
    ...
    | Case (vrnt, cases) ->
      ...
      let vrnt_cases, _ =
        try Type.get_vrnt @@ Type.beta_reduce ~deep:() tp_env vrnt_tp
        with Invalid_argument _ ->
          error tm.loc "to_type"
          @@ Printf.sprintf
               "expected variant type; found '%s'"
...
let rec subst_tp fvs sub tm =
  ...
  match tm.desc with
    ...
    | Roll (tp, tm) ->
      roll loc (Type.subst fvs sub tp) (subst_tp fvs sub tm)
    | Unroll tm ->
      unroll loc (subst_tp fvs sub tm)
    | Fix tm ->
      fix loc (subst_tp fvs sub tm)
    | Record fields ->
...
let rec subst_tm fvs sub tm =
  ...
  match tm.desc with
    ...
    | Roll (tp, tm) ->
      roll loc tp (subst_tm fvs sub tm)
    | Unroll tm ->
      unroll loc (subst_tm fvs sub tm)
    | Fix tm ->
      fix loc (subst_tm fvs sub tm)
    | Record fields ->
  ...
  let rec beta_reduce reduce_fix tp_env tm_env tm =
    ...
    match tm.desc with
      ...
      | Unroll tm ->
        ...
        (match tm'.desc with Roll (_, tm'') -> tm'' | _ -> unroll loc tm')
      | Fix tm' ->
        let tm' = beta_reduce tp_env tm_env tm' in
        if reduce_fix then
          match tm'.desc with
            | Term_abs (arg, _, body) ->
              subst_tm tm_env body arg tm
            | _ ->
              fix loc tm'
        else
          fix loc tm'
...
let alpha_equivalent =
  let rec alpha_equiv tp_env tm_env tm1 tm2 =
    match (tm1.desc, tm2.desc) with
      ...
      | Roll (tp1, tm1), Roll (tp2, tm2) ->
        Type.alpha_equivalent ~env:tp_env tp1 tp2
        && alpha_equiv tp_env tm_env tm1 tm2
      | Unroll tm1, Unroll tm2 ->
        alpha_equiv tp_env tm_env tm1 tm2
      | Fix tm1, Fix tm2 ->
        alpha_equiv tp_env tm_env tm1 tm2
  ...
  let rec simplify env tm =
    ...
    match tm.desc with
      ...
      | Roll (tp, tm) ->
        ...
        roll loc tp' tm'
      | Unroll tm ->
        unroll loc @@ simplify env tm
      | Fix tm ->
        fix loc @@ simplify env tm
      | Record fields ->
...
let rec to_string tm =
  ...
  match tm.desc with
    ...
    | Roll (tp, tm) ->
      Printf.sprintf "roll %s %s" (Type.to_string tp) (arg_to_string tm)
    | Unroll tm ->
      Printf.sprintf "unroll %s" (arg_to_string tm)
    | Fix tm ->
      Printf.sprintf "fix %s" (arg_to_string tm)
    | Record fields ->
...
let roll ?(loc = Loc.dummy) tp tm = roll loc tp tm

let unroll ?(loc = Loc.dummy) tm = unroll loc tm

let fix ?(loc = Loc.dummy) tm = fix loc tm
