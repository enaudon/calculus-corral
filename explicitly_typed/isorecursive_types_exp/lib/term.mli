...
(** [tp_abs arg kn body] constructs the abstraction of [arg] of kind [kn] from
    [body]. *)
val tp_abs : ?loc:Location.t -> Identifier.t -> Kind.t -> t -> t

(** [tp_abs' args kn body] constructs the abstraction of [args] from [body]. *)
val tp_abs' : ?loc:Location.t -> (Identifier.t * Kind.t) list -> t -> t
...
(** [subst_tp fvs sub tp] applies [sub] to [tp], replacing any variable in the
...
(** [subst_tm fvs sub tm] applies [sub] to [tm], replacing any variable in the
    domain of [sub] with the corresponding term the range of [sub]. [fvs] is any
    superset of the variables which appear in the range of [sub]. *)
