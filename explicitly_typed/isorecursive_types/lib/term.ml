...
let roll : Loc.t -> Type.t -> t -> t = fun loc tp tm ->
  { desc = Roll (tp, tm); loc }

let unroll : Loc.t -> t -> t = fun loc tm ->
  { desc = Unroll tm; loc }

let fix : Loc.t -> t -> t = fun loc tm ->
  { desc = Fix tm; loc }
...
let rec to_type (kn_env, tp_env) tm =
  ...
  match tm.desc with
    ...
    | Roll (tp, tm) ->
      ...
      in
      let ftvs = Id.Set.of_list @@ Id.Map.keys kn_env in
      let exp = Type.subst ftvs (Id.Map.singleton quant tp) body in
      let act = to_type (Id.Map.add quant kn kn_env) tp_env tm in
    ...
    | Unroll tm ->
      ...
      in
      let ftvs = Id.Set.of_list @@ Id.Map.keys kn_env in
      Type.subst ftvs (Id.Map.singleton quant tp) body
    | Fix tm ->
      let tp =
       Type.beta_reduce ~deep:() tp_env @@ to_type kn_env tp_env tm
      in
      let arg_tp, res_tp = try
        Type.get_func tp
      with Invalid_argument _ ->
        error tm.loc "to_type" @@
            Printf.sprintf
    ...
    | Variant (case, data, tp) ->
      let kn =
        Type.to_kind kn_env @@ Type.beta_reduce ~deep:() tp_env tp
      in
...
let rec to_string tm =
  ...
  let fn_to_string tm = match tm.desc with
    | Variable _ | Term_app _ | Type_app _ | Roll _ | Unroll _ | Fix _
        | Record _ | Projection _ | Variant _ ->
      to_string tm
  ...
  let arg_to_string tm = match tm.desc with
    | Variable _ | Record _ | Projection _ | Variant _ ->
      to_string tm
    | Term_abs _ | Term_app _ | Type_abs _ | Type_app _ | Roll _
        | Unroll _ | Fix _ | Case _ ->
      to_paren_string tm
  in
